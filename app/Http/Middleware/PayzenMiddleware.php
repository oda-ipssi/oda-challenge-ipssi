...
class PayzenMiddleware
{
    ...
    public function __construct($mode = 'TEST'){
        $this->payzenAccount = [
            'shopId' => config('services.payzen.shopId')
            ,'cert'   => [
                'TEST'       => config('services.payzen.testCert')
                , 'PRODUCTION' => config('services.payzen.prodCert')
            ]
            ,'mode'   => $mode
        ];
    ...
    public function simpleCreateSubscription(){
    ...
    public function buildCustomerRequest($email){
        return array(
            'billingDetails' => array('email' => $email),
        );
    }
     ...
     * @return array
     */
    public function buildSubscriptionRequest($effectDate,$amount,$currency,$initialAmount,$rrule){
        return [
